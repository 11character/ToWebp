...
public class RunMacThread implements Runnable {
	...
	public static File thisPath;				//현재위치

	public void run(){
		System.out.println("Thread : start");
		
		try {
			thisPath = new File(".").getCanonicalFile(); //현재위치를 절대경로로 팔일객체를 만든다.
			
			//생성도구 위치와 존재여부 체크
			File webpUtility = new File(thisPath.getCanonicalPath()+File.separator+"webp_utility");
